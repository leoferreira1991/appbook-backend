"""
API endpoint to fix missing book covers.
Runs the same logic as the management command but via HTTP.
"""

import time
import threading
import requests as http_requests
from urllib.parse import quote

from rest_framework.views import APIView
from rest_framework.response import Response
from rest_framework import permissions, status

from .models import Book, UserBook, UserBookExternal


def _is_valid_cover(url):
    """Verifica que la URL devuelve una imagen real (no placeholder 1x1 px)."""
    try:
        resp = http_requests.head(url, timeout=3, allow_redirects=True)
        content_length = int(resp.headers.get('content-length', '0'))
        return resp.status_code == 200 and content_length > 1000
    except Exception:
        return False


def _find_cover(title, author, isbn=None, ol_key=None):
    """
    Pipeline de busqueda de portada:
    1. Open Library por ISBN
    2. Open Library por ol_key
    3. Open Library por busqueda
    4. Google Books API
    """
    # 1. Open Library por ISBN
    if isbn:
        url = f'https://covers.openlibrary.org/b/isbn/{isbn}-M.jpg'
        if _is_valid_cover(url):
            return url

    # 2. Open Library por ol_key
    if ol_key:
        try:
            api_url = f'https://openlibrary.org{ol_key}.json'
            resp = http_requests.get(api_url, timeout=5)
            if resp.status_code == 200:
                data = resp.json()
                covers = data.get('covers', [])
                if covers:
                    cover_url = f'https://covers.openlibrary.org/b/id/{covers[0]}-M.jpg'
                    if _is_valid_cover(cover_url):
                        return cover_url
        except Exception:
            pass

    # 3. Open Library por busqueda
    try:
        query = quote(f'{title} {author}')
        resp = http_requests.get(
            f'https://openlibrary.org/search.json?q={query}&limit=1&fields=cover_i,isbn',
            timeout=5,
        )
        if resp.status_code == 200:
            data = resp.json()
            docs = data.get('docs', [])
            if docs:
                cover_id = docs[0].get('cover_i')
                if cover_id:
                    return f'https://covers.openlibrary.org/b/id/{cover_id}-M.jpg'
    except Exception:
        pass

    # 4. Google Books API
    try:
        q = quote(f'intitle:{title}+inauthor:{author}')
        resp = http_requests.get(
            f'https://www.googleapis.com/books/v1/volumes?q={q}&maxResults=1&fields=items(volumeInfo/imageLinks)',
            timeout=5,
        )
        if resp.status_code == 200:
            data = resp.json()
            items = data.get('items', [])
            if items:
                links = items[0].get('volumeInfo', {}).get('imageLinks', {})
                url = links.get('thumbnail') or links.get('smallThumbnail')
                if url:
                    return url.replace('http://', 'https://')
    except Exception:
        pass

    return None


def _get_books_without_covers():
    """Retorna queryset de UserBookExternal sin portada."""
    from django.db.models import Q

    return UserBookExternal.objects.filter(
        Q(custom_cover__isnull=True) | Q(custom_cover='')
    ).filter(
        Q(cover_url__isnull=True) | Q(cover_url='')
    ).distinct()


def _run_fix_covers_background(user_id=None):
    """
    Ejecuta la correccion de portadas en background.
    Si se pasa user_id, solo procesa libros de ese usuario.
    """
    qs = _get_books_without_covers()
    if user_id:
        qs = qs.filter(user_id=user_id)

    fixed = 0
    failed = 0

    for book in qs:
        cover_url = _find_cover(
            title=book.title,
            author=book.author,
            isbn=book.isbn,
            ol_key=book.ol_key,
        )
        if cover_url:
            book.cover_url = cover_url
            book.save(update_fields=['cover_url'])
            fixed += 1
        else:
            failed += 1
        time.sleep(0.3)

    print(f'[fix_covers] Done: {fixed} fixed, {failed} failed')
    return fixed, failed


class FixMissingCoversView(APIView):
    """
    POST /api/books/fix-covers/
    Busca portadas faltantes y las completa desde Open Library / Google Books.

    Query params:
        - mode: "check" (default) solo cuenta, "fix" ejecuta en background, "fix-sync" ejecuta sincronico
        - scope: "mine" (default) solo mis libros, "all" todos los libros (requiere staff)
    """
    permission_classes = [permissions.IsAuthenticated]

    def post(self, request):
        mode = request.data.get('mode', 'check')
        scope = request.data.get('scope', 'mine')

        # Solo staff puede procesar todos los libros
        if scope == 'all' and not request.user.is_staff:
            return Response(
                {'error': 'Solo administradores pueden procesar todos los libros.'},
                status=status.HTTP_403_FORBIDDEN,
            )

        user_id = request.user.id if scope == 'mine' else None
        qs = _get_books_without_covers()
        if user_id:
            qs = qs.filter(user_id=user_id)

        missing_count = qs.count()

        if mode == 'check':
            # Solo retornar cuantos faltan
            books_info = []
            for b in qs[:50]:  # Max 50 en el preview
                books_info.append({
                    'id': b.id,
                    'title': b.title,
                    'author': b.author,
                    'isbn': b.isbn or '',
                    'ol_key': b.ol_key or '',
                })
            return Response({
                'missing_covers': missing_count,
                'preview': books_info,
            })

        elif mode == 'fix':
            # Ejecutar en background
            thread = threading.Thread(
                target=_run_fix_covers_background,
                args=(user_id,),
                daemon=True,
            )
            thread.start()
            return Response({
                'status': 'started',
                'message': f'Buscando portadas para {missing_count} libros en segundo plano...',
                'missing_covers': missing_count,
            })

        elif mode == 'fix-sync':
            # Ejecutar sincronicamente (util para pocos libros)
            if missing_count > 20:
                return Response({
                    'error': f'Demasiados libros ({missing_count}). Usa mode=fix para ejecucion en background.',
                }, status=status.HTTP_400_BAD_REQUEST)

            fixed, failed = _run_fix_covers_background(user_id)
            return Response({
                'status': 'completed',
                'fixed': fixed,
                'failed': failed,
                'total_processed': fixed + failed,
            })

        return Response(
            {'error': 'mode debe ser "check", "fix" o "fix-sync"'},
            status=status.HTTP_400_BAD_REQUEST,
        )
